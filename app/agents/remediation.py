...
def _create_plan_agent(name: str = "remediation_planner") -> LlmAgent:
    ...
    return LlmAgent(
        ...
        instruction=(
            ...
            "STEP 3 — READ: Use execute_bash to run 'cd /tmp/workspace && cat pom.xml' "
            "to understand the project structure.\n\n"
            "STEP 4 — FIX: Use execute_bash to edit pom.xml. For BOM-managed "
            "dependencies (not listed directly in pom.xml), add a version override "
            "property using sed, e.g.:\n"
            "cd /tmp/workspace && sed -i "
            "'/<properties>/a\\    <logback.version>1.5.18</logback.version>' pom.xml\n\n"
            "STEP 5 — BUILD: Use execute_bash to run "
            "'cd /tmp/workspace && mvn -B -q -DskipTests install'. "
            "If it fails, report 'BUILD FAILURE'.\n\n"
            "STEP 6 — TEST: If build passes, run "
            "'cd /tmp/workspace && mvn -B -q verify' for tests.\n\n"
            "STEP 7 — COMMIT AND PUSH: If tests pass, run these git commands:\n"
            ...
            "STEP 8 — REPORT: Report 'BUILD SUCCESS' if everything worked, "
            "or 'BUILD FAILURE' if build or tests failed.\n\n"
            "IMPORTANT: Always prefix bash commands with 'cd /tmp/workspace && ' "
            "to ensure you're working in the cloned repo directory."
        ),
